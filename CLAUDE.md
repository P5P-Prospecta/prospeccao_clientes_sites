...
- Instalar: `/plugin marketplace add P5P-Prospecta/prospeccao_clientes_sites` → `/plugin install prospector-de-sites@p5p-prospecta`. ⚠️ O nome depois do `@` é o campo `name` de `.claude-plugin/marketplace.json` — renomeado de `arrecheneto-plugins` pra `p5p-prospecta` em 2026-08-21 (junto com `owner.name`/`author.name` → "P5P Prospecta", e o `author.name` de `prospector-de-sites/.claude-plugin/plugin.json` → "P5P Prospecta (Fábrica Digital)"). Se renomear de novo no futuro, atualizar em 4 lugares: aqui, README raiz, `prospector-de-sites/README.md`, `COMO-PUBLICAR.md` e `manual.html`.
