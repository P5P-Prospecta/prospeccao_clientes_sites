...
- [ ] **Domínio limpo e humano.** A URL padrão do GitHub Pages (`https://[usuario].github.io/[slug]/`) já é apresentável — confirme que o `slug` do cliente é legível (nada de números soltos ou hash). Se o cliente tiver domínio próprio e quiser usar no site novo, siga a seção "Domínio próprio" da skill `deploy-github-pages` (CNAME no repositório + registro DNS) antes de enviar a proposta com aquele link.
