...
2. Selecione os elegíveis: rode `list-leads proposta` (skill `supabase-sync`) e filtre quem tem `data_proposta` há **3+ dias** e SEM follow-up registrado (procure "follow-up" no campo `obs`). Se `$ARGUMENTS` indicar um cliente, restrinja a ele (mas respeite a regra de nunca repetir follow-up).
...
5. Registre em cada lead com `upsert-lead` (skill `supabase-sync`): `obs` += "Follow-up enviado em [data]".
