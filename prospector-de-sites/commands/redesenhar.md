...
5. **Atualizar**: `upsert-lead` (skill `supabase-sync`) com `status: redesenhado` pra cada cliente do lote.
...
3. **Confirmação do dashboard**: frase explícita "Dashboard atualizado: [N] leads com status redesenhado" após rodar `upsert-lead` (skill `supabase-sync`) pra cada cliente do lote — nunca pule esse passo.
