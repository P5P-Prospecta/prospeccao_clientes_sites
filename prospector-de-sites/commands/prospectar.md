...
3. **Supabase**: para cada lead, rode `upsert-lead` (skill `supabase-sync`) — leads novos entram com `status: "novo"`, descartados com `status: "descartado"` e o motivo em `obs`.

A entrega final DEVE incluir a confirmação explícita "Dashboard atualizado: [N] leads" (após rodar `upsert-lead` pra todos os leads da rodada — obrigatório, nunca pule). Mostre a tabela ao usuário com o link da planilha, e sugira o próximo passo: `/redesenhar` para os 5+ melhores leads. O painel web (login) já reflete os dados automaticamente.
